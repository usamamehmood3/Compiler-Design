from codegen import generate_new_temporary
from codegen import get_new_label
from codegen import generate_new_argument
from codegen import reset
from codegen import arg_count
...
def code_gen():
    ...
    for cid in classtable:
        c = classtable[cid]
        for (key, value) in c.fields.iteritems():
            if value.storage == 'static':
                i += 1 
    print '.static_data '+str(i)
...
def write_file(name):
    ...
    target = open(name+'.ami', 'w')
    target.write('.static_data '+str(i)+'\n')
    for cid in classtable:
        c = classtable[cid]
        for i in c.code_gen(): 
            target.write(i)
...
class Field:
    ...
    def __init__(self, fname, fclass, visibility, storage, ftype):
        ...
        self.offset = None;
    def code_gen(self):
        #Codegenerator for instance fields
        global staticFieldOffset
        if self.storage == 'instance':
            self.t = generate_new_temporary()
        elif self.storage == 'static':
            self.offset = staticFieldOffset
            staticFieldOffset += 1
        return []
...
class Method:
    ...
    def __init__(self, mname, mclass, visibility, storage, rtype):
        ...
        self.rtype = rtype
        self.vars = VarTable()
        self.label = ['M_%s_%d:'%(self.name, self.id)]
        self.code = []
    ...
    def code_gen(self):
        ...
        if not isinstance(self.body, SkipStmt):
            for s in self.body.stmtlist:
...
class VarTable:
    ...
    def get_locals(self):
        outermost = self.vars[1]  # 0 is the outermost block
        vars = [outermost[vname] for vname in outermost if outermost[vname].kind=='local']
        vars_ids = [(v.id,v) for v in vars]  # get the ids as well, so that we can order them
        vars_ids.sort()
        return [v for (i,v) in vars_ids] # in their order of definition!
...
class IfStmt(Stmt):
    def __init__(self, condition, thenpart, elsepart, lines):
        ...
        self.thenpart = thenpart #stmt
        self.elsepart = elsepart #stmt
    ...
    def code_gen(self):
        ...
        self.code = ['\n'] + \
                    self.condition.code_gen() + \
                    ['bz '+self.condition.t+', '+elselabel+':'] + \
                    self.thenpart.code_gen() + \
                    ['jmp '+ endlabel+':'] + \
                    [elselabel + ':'] + \
                    self.elsepart.code_gen() + \
                    [endlabel + ':'] + \
                    ['\n']
        return self.code
...
class WhileStmt(Stmt):
    ...
    def code_gen(self):
        self.code = []
        startlabel = get_new_label()
        endlabel = get_new_label()
        #Add the code for break and continuue
        for x in self.body.stmtlist:
            if isinstance(x, BreakStmt):
                x.make_code(endlabel)
            elif isinstance(x, ContinueStmt):
                x.make_code(startlabel)

        self.code = ['\n'] + \
                    self.cond.code_gen() + \
                    [startlabel + ':'] + \
                    ['bz '+self.cond.t+', '+endlabel+':'] + \
                    self.body.code_gen() + \
                    ['jmp ' + startlabel + ':'] + \
                    [endlabel + ':'] + \
                    ['\n'] 
...
class ConstantExpr(Expr):
    def __init__(self, kind, arg=None, lines=None):
        ...
        if (kind=='int'):
            ...
            self.string = arg
        self.code = []
        self.t = generate_new_temporary() #Sets after parsing

    #Generating Code
    def code_gen(self):
        if (self.kind=='int'):
            self.code = [ "move_immed_i %s, %s"%(self.t, self.arg) ] 
        elif (self.kind == 'float'):
            self.code = [ "move_immed_f %s, %s"%(self.t, self.arg) ]
        elif (self.kind == 'string'):
            self.code = []
        elif (self.kind == 'True'):
            self.code = ["move_immed_i %s, %s"%(self.t, str(1) )]
        elif (self.kind == 'False'):
            self.code = ["move_immed_i %s, %s"%(self.t, str(0) )]
        return self.code
...
class UnaryExpr(Expr):
    ...
    def code_gen(self):
        ...
        i1 = generate_new_temporary()
        if self.uop == 'uminus':
            if self.arg.typeof().typename == 'int':
                self.code = self.arg.code_gen() + \
                            ['move_immed_i '+i+', '+str(0)] + \
                            ['isub '+self.t+', '+i+', '+self.arg.t] 
            elif self.arg.typeof().typename == 'float':
                self.code = self.arg.code_gen() + \
                            ['move_immed_f '+i+', '+str(0)] + \
                            ['fsub '+self.t+', '+i+', '+self.arg.t] 
        elif self.uop == 'neg':
            #Expr is boolean in this case
            self.code = self.arg.code_gen() + \
                        ['move_immed_i '+i1+', '+str(1)] + \
                        ['isub '+self.t+', '+i1+', '+self.arg.t]
        return self.code
...
class BinaryExpr(Expr):
    ...
    def code_gen(self):
        bop = self.bop
        if self.typeof().typename == 'int':
            bop = 'i'+bop
            self.code = self.arg1.code_gen() + \
                        self.arg2.code_gen() + \
                        [bop + " "+ self.t + ","+ self.arg1.t + ","+ self.arg2.t]

        elif self.typeof().typename == 'float':
            bop = 'f'+bop
            self.code = self.arg1.code_gen() + \
                        self.arg2.code_gen()+ \
                        [ bop + " "+ self.t + ","+ self.arg1.t + ","+ self.arg2.t]
        elif self.typeof().typename == 'boolean':
            if (bop in ['lt', 'leq', 'gt', 'geq']):
                if self.arg1.typeof().typename == 'int' and self.arg2.typeof().typename == 'int':
                    bop = 'i'+bop
                    new1 = self.arg1.t
                    new2 = self.arg2.t
                elif self.arg1.typeof().typename == 'float' and self.arg2.typeof().typename == 'float':
                    bop = 'f'+bop
                    new1 = self.arg1.t
                    new2 = self.arg2.t
                elif self.arg1.typeof().typename == 'int' and self.arg2.typeof().typename == 'float':
                    new = generate_new_temporary()
                    bop = 'f'+bop
                    self.code = self.code + \
                                ['itof '+ new+', '+self.arg1.t]
                    new1 = new
                    new2 = self.arg2.t
                elif arg1.typeof().typename == 'float' and arg2.typeof().typename == 'int':
                    new = generate_new_temporary()
                    bop = 'f'+bop
                    self.code = self.code + \
                                ['itof '+ new+', '+self.arg2.t]
                    new1 = self.arg1.t
                    new2 = new

                code = [bop+ " "+ self.t + ","+ new1 + ","+ new2]
                self.code = self.code + \
                            self.arg1.code_gen() + \
                            self.arg2.code_gen() + \
                            code
                ...
                BZLabel = get_new_label()
                BNZLabel = get_new_label()
                t = generate_new_temporary()
                ...
                if self.arg1.typeof().typename == 'int' and self.arg2.typeof().typename == 'int':
                    ...
                    new1 = new
                    new2 = self.arg2.t
                elif self.arg1.typeof().typename == 'float' and self.arg2.typeof().typename == 'int':
                    new = generate_new_temporary()
                    sub = 'fsub '
                    self.code = self.code + \
                                ['itof '+ new+', '+self.arg2.t]
                ...
                self.code = self.code + \
                            ...
                            ['bnz ' + t + ', '+ BNZLabel + ':'] + \
                            [BZLabel + ':'] + \
                            ['move_immed_i ' + self.t + ', '+ str(val)  ] + \
                            [BNZLabel + ':'] + \
                            ['move_immed_i ' + self.t + ', '+ str(1-val)  ]

            elif bop == 'and': #Short circuit evaluation
                skip = get_new_label()
                self.code = self.arg1.code_gen() + \
                            ['move '+self.t+', '+self.arg1.t] + \
                            ['bz '+ self.t+', '+skip+':'] + \
                            self.arg2.code_gen() + \
                            ['move '+self.t+', '+self.arg2.t] + \
                            [skip+':']
            elif bop == 'or': #Short circuit evaluation
                skip = get_new_label()
                self.code = self.arg1.code_gen() + \
                            ['move '+self.t+', '+self.arg1.t] + \
                            ['bnz '+ self.t+', '+skip+':'] + \
                ...
                self.code = ['ab kia reh gia? BinExpr']
        return self.code
...
class AssignExpr(Expr):
    ...
    def code_gen(self):
        if self.lhs.mem == 'reg':
            code = ["move " + self.lhs.t + ", " +self.rhs.t]
        else:
            code = [ "hstore" + self.lhs.at + ", " + self.rhs.t ]
        self.code = self.lhs.lcode + \
                    self.rhs.code_gen() + \
                    code
        return self.code
        #Printing
...
class AutoExpr(Expr):
    ...
    def code_gen(self):
        ...
        if self.oper == 'inc':
            ...
            op = 'isub '
        i = generate_new_temporary()
        code = ['move_immed_i '+i+', '+str(1)]
        self.code = self.arg.code_gen() + \
                    code + \
                    [op+self.t+', '+self.arg.t+', '+i]
        return self.code
...
class FieldAccessExpr(Expr):
    ...
    def code_gen(self):
        if self.field.storage == 'static':
            self.code = self.code + \
                        ['add '+self.t+', '+'sap'+', '+self.offset]
        elif self.field.storage == 'instance':
            self.code = self.code + \
                        ['move '+self.t+', '+self.field.t]
...
class MethodInvocationExpr(Expr):
    ...
    def code_gen(self):
        for x in self.args:
            self.code = self.code + \
                        x.code_gen()
        global methane #Global pointer to the caller method
        formal_params = self.method.vars.get_params()
        local_vars = methane.get_locals()
        formal_params = formal_params + \
                        local_vars
        for x in formal_params:
            self.code = self.code + \
                        ['save '+x.t]
        i = 0
        for x in self.args:
            ...
            i += 1
        self.code = self.code + \
                    ['call '+self.method.label[0] ]

        #Move the return Value 
        self.code = self.code + \
                    ['move '+self.t+', '+'a0']
        for y in reversed(formal_params):
            self.code = self.code + \
                        ['restore '+y.t]
...
class NewObjectExpr(Expr):
    ...
    def code_gen(self):
        fieldcount = self.CountFields()
        for x in self.args:
            self.code = self.code + \
                        x.code_gen()
        global methane #Global pointer to the caller method
        formal_params = self.constructor.vars.get_params()
        local_vars = methane.get_locals()
        formal_params = formal_params + \
                        local_vars
        self.code = self.code + \
                    ['move_immed_i '+self.t+', '+str(fieldcount)] + \
                    ['halloc '+self.t+', '+str(fieldcount)]
        for x in formal_params:
            self.code = self.code + \
                        ['save '+x.t]
        self.code = self.code + \
...
class ArrayAccessExpr(Expr):
    def __init__(self, base, index, lines):
        ...
        self.index = index
        self.__typeof = None
        self.at = generate_new_temporary()
        self.t = generate_new_temporary()
        self.mem = 'heap'
        self.rcode = []
        self.lcode = []
    ...
    def code_gen(self):
        code = [ "add " +self.at +', ' + self.base.t + ', '+self.index.t ]
        self.lcode = self.base.code_gen() +  \
                    self.index.code_gen() +  \
                    code

        code = [ "hload "+ self.t+', '+self.at ]
...
class NewArrayExpr(Expr):
    def __init__(self, basetype, args, lines):
        ...
        self.t = generate_new_temporary()
    def code_gen(self):
        #Code for computings arguments and putting in their registers
        for x in self.args:
            self.code = self.code + \
                        x.code_gen()
        #------------------------------------------------------------
        ...
        if len(self.args) == 1:
            ...
            mulr = generate_new_temporary()
            self.code = self.code +\
                        [ "imul "+mulr+', '+ self.args[0].t+ ', '+self.args[1].t ]
        else:
            mulr = generate_new_temporary()
            self.code = self.code +\
                        [ "imul "+mulr+', '+ self.args[0].t+ ', '+self.args[1].t ]
            for x in range(2, len(self.args)):
                self.code = self.code + \
                            ['imul '+mulr+', '+mulr+', '+self.args[x].t]

        code = [ "halloc " + self.t + ", "+ mulr ]
